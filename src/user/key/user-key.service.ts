...
import type {
    ...
    LokalAuthSaltedKey,
    LokalAuthSaltedKeyService,
} from "../../key/types.ts";

export class UserKeyService implements LokalAuthSaltedKeyService {
    ...
    async fromJSON(
        ...
        const key = await crypto.subtle.importKey(
            ...
            true,
            [...UserKeyService.DERIVE_OPTIONS.key.operations],
        );
        return {
            cryptoKey: key,
    ...
    private async initKey(
        ...
        const deriveKey = await globalThis.crypto.subtle.importKey(
            "raw",
            passBuffer,
            { name: UserKeyService.DERIVE_OPTIONS.deriveKeyAlgorithm },
            false,
        ...
        const key = await globalThis.crypto.subtle.deriveKey(
            {
                name: UserKeyService.DERIVE_OPTIONS.deriveKeyAlgorithm,
                salt: salt,
                iterations: UserKeyService.DERIVE_OPTIONS.key.iterations,
                hash: UserKeyService.DERIVE_OPTIONS.key.hash,
            },
            deriveKey,
            {
                name: UserKeyService.DERIVE_OPTIONS.key.algorithm,
                length: UserKeyService.DERIVE_OPTIONS.key.length,
            },
            true,
            [...UserKeyService.DERIVE_OPTIONS.key.operations],
        );
        ...
        return {
            ...
            options: {
                ...
                version: "001",
                key: {
                    algorithm: UserKeyService.DERIVE_OPTIONS.key.algorithm,
                    length: UserKeyService.DERIVE_OPTIONS.key.length,
                },
            },
        };
